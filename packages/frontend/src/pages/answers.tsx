...
const MyAnswers: NextPage = () => {
  ...
  const { checkCurrentLensSession } = useSessionClient();
  ...
  useEffect(() => {
    const loadPosts = async () => {
      ...
      if (!state.stateActiveLensProfile) {
        ...
        setError("No lens profile active");
        return
      }
      try {
    ...
    "
        ...
        <div className="mt-10">
          ...
          {!isLoading && !error && publications.length > 0 && (
            <div className="space-y-6">
              {(() => {
                const hasAccepted = publications.some(
                  (p) => p.commentOn.metadata?.attributes?.[0]?.value === me
                );

                return publications.map((p) => {
                  const author = p.commentOn.metadata?.attributes?.[0]?.value;
                  const isAccepted = author === me;
